...
const StepperControlIndigency = ({ 
  handleClick, 
  currentStep, 
  steps, 
  uploadIDFieldsComplete, 
  personalInfoFieldsComplete, 
  indigencyAddressFieldsComplete, 
  ...
  const handleNextClick = async () => {
    if (currentStep === 1 && !uploadIDFieldsComplete) {
      setErrorMessage('Upload Government ID first.');
      return;
    } else if (currentStep === 2 && !personalInfoFieldsComplete) {
      setErrorMessage('Personal information fields are incomplete.');
      return;
    } else if (currentStep === 3 && !indigencyAddressFieldsComplete) {
  ...
  }, [uploadIDFieldsComplete, personalInfoFieldsComplete, indigencyAddressFieldsComplete, paymentFieldsComplete, currentStep]);
      ...
      <div className="flex justify-around w-full">
        ...
        <button
          ...
          className={`next-button uppercase py-2 px-4 rounded-xl font-semibold cursor-pointer transition duration-200 ease-in-out 
            ${(!uploadIDFieldsComplete && currentStep === 1) || (!personalInfoFieldsComplete && currentStep === 2) || (!indigencyAddressFieldsComplete && currentStep === 3) || (!paymentFieldsComplete && currentStep === 4) ? "bg-green-200 text-gray-700 cursor-not-allowed" : "bg-green-500 text-white hover:bg-green-900 hover:text-white"}`}
        >
...
export default StepperControlIndigency;
